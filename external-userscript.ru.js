...
Я предлагаю разместить инструменты разработчика в правой части окна браузера, если они еще не там (посмотрите здесь, если не знаете как: https://www.google.com/search?q=developer+tools+dock+side&udm=2).
...
Затем кликните на вывод объекта (обёрнутый в фигурные скобки) в консоли ниже, чтобы увидеть его свойства.
...
JSD.thenExpandMe.nextStep = {
  recommendation1: "Введите JSD.thenExpandMe.nextStep.abraCadabra - прочитаете волшебное свойство!",
  recommendation2: "Воздержитесь от погружения в кроличью нору",
  recommendation3: "бесконечного разворачивания прототипа ниже.",
  __proto__: {
    get abraCadabra() {
      window.run = run
      console.log(text2)
...
Нам предстоит вызвать ещё немало функций, использовать кучу переменных, развернуть/свернуть массу объектов, чтобы прочитать сотни свойств.
Но перед этим давайте подытожим несколько моментов:
...
Текст, который вы здесь пишете, называется "код". Он рассматривается как ваш "ввод", как только вы нажимаете Enter. А ответ интерпретатора называется "вывод". Он отображается в консоли под вашим вводом. Обратите внимание на маркер ❯ перед вашим вводом и маркер ❮• перед выводом консоли. Есть также сообщения, подобные этому, без каких-либо маркеров слева, но со ссылкой справа, ведущей к исходному коду JavaScript Discovery. Если вы кликнете по ссылке, вы будете перенаправлены на вкладку Source (Источники), где будет показан исходный код этого скрипта. А чтобы вернуться сюда, вам нужно будет кликнуть на вкладку Console (Консоль) выше. Там много кода, так что я не уверен, что читать его сейчас - хорошая идея. Вы всегда сможете сделать это позже, когда будете готовы.

Вызовите функцию 'run()' снова, чтобы продолжить.`

const text4 = raw`Чуть ранее вы ввели 'JSD' и нажали Enter (если вы следовали моим инструкциям, что, надеюсь, так). Это уже был крошечный фрагмент кода, один токен кода, идентификатор глобальной переменной с именем JSD. И став вашим вводом, этот единственный идентификатор переменной сформировал одно из простейших выражений, которые может вычислить интерпретатор JavaScript - выражение чтения переменной. Таким образом, он получил доступ к значению объекта, привязанному к этой переменной в памяти, и вывел в консоль его представление в общепринятом формате: с фигурными скобками и свойствами (разделенными запятыми), представленными именами свойств и значениями (разделенными двоеточиями).

Итак, вы прочитали значения свойств с именами 'readMe1', 'readMe2' и 'readMe3', и этими значениями были строки текста. Затем вы получили доступ к значению свойства с именем 'thenExpandMe', развернув его кликом. И этим значением был другой объект со своим набором свойств с именами 'hintN' с 1 по 5 с еще пятью строками текста в качестве их значений. Из этих подсказок вы узнали, что помимо разворачивания объектов в консоли вы также можете получать доступ/читать их значения напрямую с помощью выражений вроде 'имяПеременной.имяСвойства' или даже 'имяПеременной.имяСвойства.имяСвойства' (вплоть до любой глубины).

В выражении вроде 'a.b' и 'a', и 'b' являются идентификаторами. 'a' должен идентифицировать некоторую существующую переменную, в то время как 'b' должен идентифицировать свойство с именем 'b' объекта, привязанного к переменной с именем 'a'. А точка между 'a' и 'b' называется "оператором доступа к свойству" (property access operator) или "точечной нотацией" (dot notation). Таким образом, 'a.b' - это выражение чтения свойства объекта. И результатом вычисления этого выражения является значение, привязанное к свойству с именем 'b' объекта, привязанного к переменной с именем 'a'.
...
Объясняя все это, я упомянул несколько терминов, которые важно понять: идентификатор (identifier), оператор (operator), токен (token), выражение (expression), переменная (variable), свойство (property), привязка (binding), строка (string), объект (object), значение (value) и функция (function). Я поместил объект с некоторыми определениями/объяснениями этих терминов в новое свойство объекта 'JSD'. Вы можете попробовать прочитать их, снова получив доступ к 'JSD' (поскольку вы не увидите его в устаревшем выводе, оставшемся выше) и развернув свойство 'terms', или можно ввести 'JSD.terms' и нажать Enter. Но эти строки довольно длинные, и они обрезаются консолью, а читать их через маленькие всплывающие подсказки при наведении не очень удобно. Поэтому я представляю вам функцию с именем 'log', которая будет выводить текст в консоль в более читаемом виде. Вы можете вызвать ее с помощью выражения вроде 'log(любое выражение)'. Попробуйте написать в скобках вызова выражение чтения свойства, например 'JSD.terms.identifier'.
...
terms.identifier = `Идентификатор (identifier) — это токен кода, именующий сущность с привязкой, такую как переменная, свойство или функция. Он также может именовать константу, параметр, метод или класс, но мы их еще не рассматривали.
...
Строго говоря, переменные, свойства или функции (или любые другие сущности с привязкой) сами по себе не существуют в коде. В коде лишь токены, обращающиеся к ним по имени, и они называются идентификаторами.`

terms.operator = `Оператор (operator) — это токен кода, состоящий из одного символа или комбинации символов, который выполняет операцию над одним или несколькими операндами. Выше мы использовали оператор доступа к свойству '.' и оператор вызова функции '()'.
...
terms.token = `Токен (token) — это отдельная единица кода, такая как идентификатор или оператор, по аналогии с тем, как слова и знаки препинания являются токенами в человеческих языках. В JS есть и другие типы токенов, такие как ключевые слова, литералы, пунктуация и комментарии, но мы их еще не рассматривали.
...
terms.expression = `Выражение (expression) — это допустимый сегмент кода, который вычисляется в одно значение, говорят "возвращает значение". Подобно математическому выражению 2 + 2, которое вычисляется в 4. Выражения могут быть простыми или сложными, и они могут включать множество вложенных подвыражений.

Например:
  - Одиночный идентификатор, такой как 'JSD', — это выражение, возвращающее объект, хранящийся в переменной JSD.
  - Доступ к свойству, такой как 'JSD.readMe1', — это выражение, возвращающее строковое значение этого свойства.
  - Вызов функции, такой как 'run()', — это выражение, которое выполняет функцию и разрешается в ее возвращаемое значение (в данном случае undefined).
  - Более длинные комбинации токенов, такие как 'JSD.thenExpandMe.nextStep.abraCadabra', также образуют выражения.

Всякий раз, когда вы вводите что-либо в консоль и нажимаете Enter, интерпретатор пытается вычислить это как выражение (или как инструкцию, содержащую выражения) и обычно отображает результирующее значение (вывод после '❮•').`

terms.variable = `Переменная (variable) — это самая знакомая категория сущностей с привязкой. Это именованный "контейнер" в памяти компьютера, который вы связываете со значением. Он позволяет хранить данные и ссылаться на них, используя осмысленное имя (идентификатор).
...
Вы получали к ним доступ с помощью оператора точки '.', например, 'JSD.readMe1', или с вложенностью, как 'JSD.thenExpandMe.hint4'.

Каждое свойство состоит из "ключа" (имени, например 'readMe1' или 'hint1') и связанного значения, которое может быть любого типа данных JavaScript, включая другие объекты, функции, строки, числа и т.д.`
...
Помимо переменных (variable) и свойств (property), существуют и другие категории привязок, такие как константы (constant), функции (function), параметры (parameter), методы (method) и классы (class). Хотя некоторые из этих понятий пересекаются или тесно связаны, все эти термины активно используются. Они различаются в основном тем, как они объявляются (часто с использованием разных ключевых слов) и используются.`

terms.string = `Строка (string) — это один из базовых, примитивных типов данных в JavaScript, используемый для представления текстовых данных. По сути, это упорядоченная последовательность из нуля или более символов. Чтобы создать строковое значение в вашем коде, вы заключаете последовательность символов в одинарные кавычки ('вот так'), двойные кавычки ("вот так") или обратные кавычки (\`вот так\`). Это представление в коде (символы плюс окружающие кавычки или обратные кавычки) называется "строковым литералом".

Вы уже сталкивались со многими строковыми значениями. Свойства 'readMe', подсказки внутри 'JSD.thenExpandMe' и рекомендации в 'JSD.thenExpandMe.nextStep' — все они хранились как строки. Вы могли заметить, что консоль также для удобства показывает строковые значения обернутыми в кавычки.
...
Существует так много типовых объектов, что у них есть еще один уровень категоризации, называемый "классами" или "типами объектов". Функции и массивы являются наиболее распространенными типами типизированных объектов.`

terms.value = `Значение (value) представляет собой информацию или фактические данные, с которыми работает ваша программа. Это конкретный фрагмент данных, который является результатом вычисления выражения и часто хранится в привязке (например, в переменной или свойстве). Каждое значение имеет определенный тип данных, который определяет, что это за данные и какие операции с ними можно выполнять.

В JavaScript есть семь примитивных типов значений: строки (string, текст, например "hello"), числа (number, например 42 или 3.14), булевы значения (boolean, логические true или false), 'undefined' (представляющий отсутствие определенного значения), 'null' (представляющий намеренное отсутствие значения), 'symbol' и 'bigint'. Объекты (включая функции, массивы и многие другие) являются восьмым - сложным типом данных.

Мы создаем значения литералами (либо как одиночные токены, либо как сложные литералы, состоящие из нескольких токенов) или с помощью "функций-конструкторов" (также часто называемых "классами"). Но мы можем получать значения и из других выражений.`

terms.function = `Функция (function) — это базовая единица решения задач в JavaScript. По сути, это многократно используемый блок кода, предназначенный для выполнения определенной задачи или вычисления значения. Функции позволяют инкапсулировать фрагмент логики, часто присваивая ему имя через привязку (идентификатор), чтобы вы могли выполнять эту логику несколько раз, не переписывая код, просто "вызывая" функцию.

Вы вызываете или выполняете функцию, используя ее имя, за которым следуют круглые скобки '()' (оператор вызова функции). Внутри скобок вы можете опционально передавать аргументы (значения), с которыми функция будет работать, хотя функция 'run', которую вы использовали, не требует аргументов.

Функции сами по себе являются значениями (фактически, они являются особым типом объекта). Это означает, что их можно присваивать переменным (как это было с переменной 'run'), хранить как свойства объекта (где их обычно называют "методами") и передавать так же, как любое другое значение, что делает их очень мощными и гибкими.`
...
function run() {
  ...
  const substitute = run.count < 3
    ...
    : `Снова... уже ${run.count} раз на самом деле!`
