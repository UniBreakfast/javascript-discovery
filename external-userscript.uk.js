...
Пояснюючи все це, я використав кілька термінів, які важливо зрозуміти: ідентифікатор (identifier), оператор (operator), токен (token), вираз (expression), змінна (variable), властивість (property), прив'язка (binding), рядок (string), об'єкт (object), значення (value) та функція (function). Я помістив об'єкт з деякими визначеннями/поясненнями цих термінів у нову властивість об'єкта 'JSD'. Ви можете спробувати прочитати їх, знову отримавши доступ до 'JSD' (оскільки ви не побачите його в застарілому виведенні, що залишилося вище) і розгорнувши властивість 'terms', або можна ввести 'JSD.terms' і натиснути Enter. Але ці рядки досить довгі, і вони обрізаються консоллю, а читати їх через маленькі спливаючі підказки при наведенні не дуже зручно. Тому я пропоную вам функцію з ім'ям 'log', яка буде виводити текст у консоль у більш читабельному вигляді. Ви можете викликати її за допомогою виразу на зразок 'log(будь-який вираз)'. Спробуйте написати в дужках виклику вираз читання властивості, наприклад 'JSD.terms.identifier'.
...
terms.token = `Токен (token) — це окрема одиниця коду, така як ідентифікатор або оператор, за аналогією з тим, як слова та розділові знаки є токенами в людських мовах. У JS є й інші типи токенів, такі як ключові слова, літерали, пунктуація та коментарі, але ми їх ще не розглядали.
...
Ви вже використовували змінні:
  ...
  - 'run' — це інша глобальна змінна, що в якості значення містить функцію.
...
Крім змінних (variable) та властивостей (property), існують й інші категорії прив'язок, такі як константи (constant), функції (function), параметри (parameter), методи (method) та класи (class). Хоча деякі з цих понять перетинаються або тісно пов'язані, всі ці терміни активно використовуються. Вони різняться в основному тим, як вони оголошуються (часто з використанням різних ключових слів) та використовуються.`
...
terms.value = `Значення (value) представляє собою інформацію або фактичні дані, з якими працює ваша програма. Це конкретний фрагмент даних, який є результатом обчислення виразу і часто зберігається у прив'язці (наприклад, у змінній або властивості). Кожне значення має певний тип даних, який визначає, що це за дані та які операції з ними можна виконувати.
...
Ми створюємо значення літералами (або як одиночні токени, або як складні літерали, що складаються з декількох токенів) або за допомогою "функцій-конструкторів" (їх також називають "класами"). Але ми можемо отримувати значення і з інших виразів.`
